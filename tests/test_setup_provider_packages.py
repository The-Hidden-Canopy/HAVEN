...
from haven.web.provider_install import find_installed_provider, load_installed_provider_config
...
def test_install_provider_package_activates_it_and_sets_provider_kind(monkeypatch):
    ep = metadata.EntryPoint(name="philips_hue", value=f"{__name__}:FAKE_HUE_PLUGIN", group="haven.providers")
    _patch_entry_points(monkeypatch, ep)
    with tempfile.TemporaryDirectory() as tmp:
        data_dir = Path(tmp)
        ...
        store = SetupConfigStore(data_dir / "haven.json")
        assert store.load().provider_kind == "philips_hue"
...
def test_uninstall_provider_package_never_resets_provider_kind(monkeypatch):
    """Regression: clearing provider_kind on uninstall would resurface the
    demo fixture for a household with real enrolled devices -- the exact
    silent fallback this wave's item 3 removed."""
